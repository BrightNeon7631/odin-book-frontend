...
const getLatestUserPostReplies = async (
  userId: string | number,
  pageParam?: number | null,
) => {
  const res = await axios.get<PostAllData[]>(
    `/post/latest/user/${userId}/replies?cursor=${pageParam}`,
  );
  await fakeLoading(1000);
  return res.data;
};

export default function UserPostReplies() {
  ...
  const {
    ...
    queryKey: ['latestUserPostReplies', userId],
    queryFn: ({ pageParam = 0 }) => {
      if (!userId) {
        throw new Error('ID is required.');
      }
      return getLatestUserPostReplies(userId, pageParam);
    },
    initialPageParam: 0,
